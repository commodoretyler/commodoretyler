...
class Commodore_Tyler_Ephemera_Widget extends WP_Widget {
	 ...
	 * @since Commodore Tyler 1.0
	 *
	 * @return Commodore_Tyler_Ephemera_Widget
	 */
	public function __construct() {
		parent::__construct( 'widget_commo_ephemera', __( 'Commodore Tyler Ephemera', 'commo' ), array(
			'classname'   => 'widget_commo_ephemera',
			'description' => __( 'Use this widget to list your recent Aside, Quote, Video, Audio, Image, Gallery, and Link posts', 'commo' ),
		) );
		...
		$this->format_strings = array(
			'aside'   => __( 'Asides',    'commo' ),
			'image'   => __( 'Images',    'commo' ),
			'video'   => __( 'Videos',    'commo' ),
			'audio'   => __( 'Audio',     'commo' ),
			'quote'   => __( 'Quotes',    'commo' ),
			'link'    => __( 'Links',     'commo' ),
			'gallery' => __( 'Galleries', 'commo' ),
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $ephemera->have_posts() ) :
				...
				<article <?php post_class(); ?>>
					<div class="entry-content">
						...
						<p class="wp-caption-text">
							<?php
								printf( _n( 'This gallery contains <a href="%1$s" rel="bookmark">%2$s photo</a>.', 'This gallery contains <a href="%1$s" rel="bookmark">%2$s photos</a>.', $total_images, 'commo' ),
			...
			<a class="post-format-archive-link" href="<?php echo esc_url( get_post_format_link( $format ) ); ?>"><?php printf( __( 'More %s <span class="meta-nav">&rarr;</span>', 'commo' ), $this->format_strings[ $format ] ); ?></a>
			<?php
	...
	function form( $instance ) {
		...
		?>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Title:', 'commo' ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>"></p>

			<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php _e( 'Number of posts to show:', 'commo' ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" size="3"></p>

			<p><label for="<?php echo esc_attr( $this->get_field_id( 'format' ) ); ?>"><?php _e( 'Post format to show:', 'commo' ); ?></label>
